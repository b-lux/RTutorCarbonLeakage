...
#! addon__quiz__Introduction of the EU ETS

If you want to find out more about the development of the EU ETS have a look at the ETS Summer University. The European Commission has set up a comprehensive online course on the background and design of Emission Trading Systems, [here](https://ec.europa.eu/clima/policies/ets/ets-summer-university/content/ets-e-learning-online-course).
...
```{r "1_Current_Allocation__19",eval=FALSE}
...
```{r "2_The_Vulnerability___3",optional=TRUE}
...
```{r "2_The_Vulnerability___12",eval=FALSE}
...
So far we have made very broad assumptions on the relocation probability function $F_i$ to develop the model. In order to apply the model to data, we need to become more specific. Martin et al. assumed firm $i$'s profit increases linearly in the amount of free permits it receives: 
$$
...
$$
Have a look at the exact mapping between the VS and relocation probabilities as introduced in Exercise 2 in the *info*-box below.
...
$$
F_{ex}(b_{ex}) = \frac{1}{1+\exp (-4.595 + 5.635\cdot 10^{-6}\cdot batchsize\cdot b_{ex})}.\qquad (3.1.9)
$$
...
As we could end up in all other states of firm 2 (depending on the allocation decision at firm 1), similar calculations need to be done for all possible values of $rest_2 \in (0,\cdots,5)$. Have a look at Figure 3.2.1 again and answer the following question:
...
```{r "3_3___Optimal_Alloca__6",optional=TRUE}
...
```{r "3_3___Optimal_Alloca__12",optional=TRUE, eval=FALSE}
...
```{r "3_3___Optimal_Alloca__13",optional=TRUE, eval=FALSE}
# Load the data necessary to run the optimization. 
simulation.data = read.dta("data/simulationdata.dta") %>%
  rename(grandf = allo)
# %>% arrange(desc(beta1))
...
```{r "3_4___Optimal_Alloca__6",optional=TRUE, eval=FALSE}
# Load the data necessary to run the optimization. 
# Order the data in descending order with respect to beta1.
simulation.data = read.dta("data/simulationdata.dta") %>%
  rename(grandf = allo) %>%
...
```{r "4_Feasible_Optimal_A__6",optional=TRUE}

```

#! addon__quiz__First feasible allocation
...
#! addon__quiz__Third feasible allocation rule
...
```{r "4_Feasible_Optimal_A__11",eval=FALSE}
...
* Martin, R., Muûls, M., de Preux, L. and Wagner, U. (2014b). On the empirical content of carbon leakage criteria in the EU Emissions Trading Scheme. Ecological Economics, 105, pp.78-88.
